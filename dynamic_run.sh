...
# ==========================================================
    ...
    python dynamic_ixia.py ${1} ${2} ${id} & # start the python  
	...
	ps -ef|grep dynamic_ixia.py|grep -v grep|cut -c 9-15|xargs kill -9 # kill python process
...
if [[ -z $2 ]]; then
    echo "Please input mode type and try again"
    exit
fi
